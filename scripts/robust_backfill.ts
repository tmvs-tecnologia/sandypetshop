...
async function robustBackfill() {
    ...
    for (const client of clients) {
        ...
        if (client.recurrence_type === 'monthly') {
            ...
            if (candidate.getDate() > client.recurrence_day) {
                candidate.setMonth(candidate.getMonth() + 1);
            }
            candidate.setDate(client.recurrence_day);
        } else {
            // Weekly or Bi-weekly
        ...
        if (currentDate < startDate) {
            currentDate = getNextDate(currentDate, client.recurrence_type, client.recurrence_day);
        }
